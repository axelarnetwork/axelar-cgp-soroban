...
pub fn validate_proof(env: &Env, data_hash: BytesN<32>, proof: Proof) -> bool {
    ...
    let signers_epoch = get_signers_epoch(env, &signers_hash)
...
pub fn rotate_signers(env: &Env, new_signers: &WeightedSigners, enforce_rotation_delay: bool) {
    ...
    if get_signers_epoch(env, &new_signers_hash).is_some() {
...
fn get_signers_epoch(env: &Env, signers_hash: &BytesN<32>) -> Option<u64> {
    env.storage()
        .persistent()
